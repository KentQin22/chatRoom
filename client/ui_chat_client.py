...
class Login_Window():
    ...
    def __init__(self, login_action, regist_action, socket):  # 需要传入登录方法
    ...
    def _login_action(self):
        ...
        self.login_action(self.user.get(), self.passwd.get(), self.socket)  # 调用传入的login_action方法

    def _regist_action(self):
        self.login_screen.withdraw()
        regist_screen = Regist_Window(self.regist_action, self.socket)
...
class Regist_Window():
    ...
    def __init__(self, regist_action, socket):  # 需要传入注册方法
    ...
    def _regist_action(self):
        ...
        if passwd == "" or passwd is None:
            msg_box_err("警告", "密码不能为空!")
            return
        self.regist_action(user, passwd, self.socket)
